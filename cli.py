...
def publish_now(
    ...
    if caption:
        ...
        if content_type == "image":
            ...
            media_path = create_image_card(card_title, badge, bullets, f"live_{task_id}.png")
            ...
            media_path = create_vertical_video(video_title, slides, audio_path, f"live_video_{task_id}.mp4")
...
def list_tasks():
    ...
    header = f"{'ID':<6} | {'Publish Time':<17} | {'Type':<10} | {'Recurrence':<11} | {'Status':<18} | {'Topic / Caption':<35}"
    print("-" * len(header))
    print(header)
    print("-" * len(header))
    for t in tasks:
        display_text = t.get('custom_caption', '').replace('\n', ' ') if t.get('custom_caption') else t.get('topic_title', '')
        print(f"{t.get('id', ''):<6} | {t.get('publish_time', ''):<17} | {t.get('content_type', ''):<10} | {t.get('recurrence', 'none'):<11} | {t.get('status', ''):<18} | {display_text[:33]:<35}")
    print("-" * len(header))
...
def add_task(
    ...
    """Appends a new task row to schedule.csv with optional custom caption and recurrence."""
    tasks = read_tasks()
    ...
    logger.info(f"✅ নতুন টাস্ক যোগ করা হয়েছে: ID={next_id}, Topic='{topic}', Time={publish_time}{rec_info}{cap_info}")
...
def main():
    ...
    if args.command == "run":
        ...
        publish_now(args.title, args.type, args.keywords, args.platforms, args.caption)
        ...
        add_task(args.title, args.type, args.time, args.keywords, args.platforms, args.caption, args.recurrence)
    else:
        parser.print_help()
